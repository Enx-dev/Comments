...
const Replies = ({
  ...
  const [deleteModel, setDelete] = useState<boolean>(false);
  ...
  const UpdateComment = () => {
    if (update) {
      dispatch(updateComment({ type: "reply", id, parentId, content: update }));
      setEdit(false);
    } else {
  ...
  const AddScore = () => {
    if (Score < 1) {
      ...
      dispatch(increaseScore({ id, type: "reply", parentId }));
    }
  };
  const MinusScore = () => {
    if (Score > -1) {
      setScore((prev) => prev - 1);
      dispatch(DecreaseScore({ id, type: "reply", parentId }));
    }
  };
  return (
    <>
        ...
        <div className='Comment_top'>
          ...
          <div className='Comment_top_content'>
            {edit ? (
              ...
              <p className='Comment_top_content_p'>
